...
import {
  ...
  ShoppingCart, UserCheck, UserX, Timer,
} from 'lucide-react'
import { analyticsApi, suppliersApi, inventoryApi, customersApi } from '../lib/api'
...
export default function AIInsightsPage() {
  ...
  // ── New: Stockout + Customer queries ───────────────────────
  const { data: lowStockRaw }  = useQuery({
    queryKey: ['ai-lowstock'],
    queryFn:  () => inventoryApi.lowStock(),
    staleTime: 60_000,
  })

  const { data: topCustomersRaw } = useQuery({
    queryKey: ['ai-top-customers'],
    queryFn:  () => customersApi.list({ sortBy: 'totalSpent', sortDir: 'desc', limit: 5 }),
    staleTime: 60_000,
  })

  const { data: atRiskRaw } = useQuery({
    queryKey: ['ai-at-risk-customers'],
    queryFn:  () => customersApi.list({ sortBy: 'lastPurchase', sortDir: 'asc', limit: 10 }),
    staleTime: 60_000,
  })
  ...
  const lowStockList  = Array.isArray(lowStockRaw) ? lowStockRaw : []
  const topCustomers  = Array.isArray(topCustomersRaw) ? topCustomersRaw : (topCustomersRaw?.data ?? [])
  const atRiskRawList = Array.isArray(atRiskRaw)       ? atRiskRaw       : (atRiskRaw?.data        ?? [])
  ...
  const stockoutPredictions = useMemo(() => {
    return topProducts
      .map((p: any) => {
        const soldQty     = Number(p.quantity ?? p.qty ?? p.total_sold ?? 0)
        const salesPerDay = soldQty / periodDays
        // Match with low stock items by product name or id
        const stockItem   = lowStockList.find((s: any) =>
          s.variant?.product?.name === (p.productName ?? p.name) ||
          s.variant?.product?.id   === p.productId
        )
        const currentQty  = stockItem ? Number(stockItem.quantity) : null
        const daysLeft    = (currentQty !== null && salesPerDay > 0)
          ? Math.floor(currentQty / salesPerDay)
          : null
  ...
  }, [topProducts, lowStockList, periodDays])

  // ── Customer at-risk (no purchase > 30 days) ────────────
  const atRiskCustomers = useMemo(() => {
    const threshold = dayjs().subtract(30, 'day')
    return atRiskRawList.filter((c: any) => {
      const last = c.lastPurchaseAt ?? c.updatedAt
      return last && dayjs(last).isBefore(threshold)
    }).slice(0, 5)
  }, [atRiskRawList])
      ...
      {loading ? (
        ...
        <>
          ...
          {stockoutPredictions.length > 0 && (
            <div className="bg-surface border border-border rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                <Timer size={16} className="text-rose" />
                <h3 className="font-semibold text-fg text-sm">Stockout Predictions</h3>
                <span className="text-[10px] bg-rose/10 text-rose px-2 py-0.5 rounded-full font-semibold">
                  {stockoutPredictions.filter(p => p.severity !== 'ok').length} at risk
                </span>
                <span className="text-[10px] text-muted ml-auto">Based on {periodDays}-day sales rate</span>
              </div>
              <div className="space-y-2">
                {stockoutPredictions.map((p, i) => {
                  const sev = {
                    critical: { bar: 'bg-rose',        badge: 'text-rose bg-rose/10',         label: 'CRITICAL' },
                    warning:  { bar: 'bg-orange-400',  badge: 'text-orange-400 bg-orange-400/10', label: `${p.daysLeft}d left` },
                    low:      { bar: 'bg-yellow-400',  badge: 'text-yellow-400 bg-yellow-400/10', label: `${p.daysLeft}d left` },
                    ok:       { bar: 'bg-jade',        badge: 'text-jade bg-jade/10',           label: `${p.daysLeft}d left` },
                  }[p.severity]

                  const barW = p.daysLeft !== null ? Math.min(100, (p.daysLeft / 30) * 100) : 100
                  return (
                    <div key={i} className="flex items-center gap-3 py-2 border-b border-border/40 last:border-0">
                      <Package size={13} className="text-muted flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="font-medium text-fg truncate max-w-[50%]">{p.name}</span>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-muted">{p.currentQty} in stock · {p.salesPerDay}/day</span>
                            <span className={clsx('text-[10px] font-bold px-1.5 py-0.5 rounded-full', sev.badge)}>
                              {p.severity === 'critical' ? '⚠ ' + sev.label : sev.label}
                            </span>
                          </div>
                        </div>
                        <div className="h-1 bg-surface2 rounded-full overflow-hidden">
                          <div
                            className={clsx('h-full rounded-full transition-all', sev.bar)}
                            style={{ width: `${barW}%` }}
                          />
                        </div>
                      </div>
              ...
              {stockoutPredictions.some(p => p.severity === 'critical') && (
                <div className="mt-3 flex items-center gap-2 text-xs text-rose bg-rose/5 rounded-lg px-3 py-2 border border-rose/20">
                  <AlertTriangle size={12} />
                  Critical items need restocking immediately — contact your supplier
                </div>
              )}
            </div>
          )}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {topCustomers.length > 0 && (
              <div className="bg-surface border border-border rounded-xl p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-semibold text-fg text-sm">Top Customers</h3>
                  <span className="text-[10px] text-muted bg-surface2 px-2 py-0.5 rounded-full">By total spend</span>
                </div>
                <div className="space-y-2.5">
                  {topCustomers.slice(0, 5).map((c: any, i: number) => {
                    const spent   = Number(c.totalSpent ?? 0)
                    const maxSp   = Number(topCustomers[0]?.totalSpent ?? 1)
                    const barW    = maxSp > 0 ? (spent / maxSp) * 100 : 0
                    const lastBuy = c.lastPurchaseAt ?? c.updatedAt
                    return (
                      <div key={c.id ?? i}>
                        <div className="flex items-center justify-between text-xs mb-0.5">
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="w-5 h-5 rounded-full bg-jade/20 text-jade flex items-center justify-center text-[10px] font-bold flex-shrink-0">
                              {i + 1}
                            </span>
                            <span className="font-medium text-fg truncate">{c.name ?? c.phone ?? '—'}</span>
                            {c.loyaltyPoints > 0 && (
                              <span className="text-[10px] text-gold bg-gold/10 px-1.5 rounded-full flex-shrink-0">
                                ⭐ {c.loyaltyPoints}pt
                              </span>
                            )}
                          </div>
                          <div className="text-right flex-shrink-0">
                            <span className="font-mono font-bold text-fg">{fmt.compact(spent)}</span>
                            {lastBuy && (
                              <span className="block text-[10px] text-muted">{dayjs(lastBuy).fromNow()}</span>
                            )}
                          </div>
                        </div>
                        <div className="h-1 bg-surface2 rounded-full overflow-hidden ml-7">
                          <div className="h-full bg-jade/50 rounded-full" style={{ width: `${barW}%` }} />
                        </div>
                      </div>
                    )
                  })}
            ...
            <div className="bg-surface border border-border rounded-xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-semibold text-fg text-sm">At-Risk Customers</h3>
                <span className="text-[10px] text-muted bg-surface2 px-2 py-0.5 rounded-full">No purchase &gt; 30 days</span>
              </div>
              {atRiskCustomers.length === 0 ? (
                <div className="flex items-center gap-2 text-jade text-sm">
                  <Star size={14} />
                  <span>All customers are active!</span>
                </div>
              ) : (
                <div className="space-y-2">
                  {atRiskCustomers.map((c: any, i: number) => {
                    const lastBuy  = c.lastPurchaseAt ?? c.updatedAt
                    const daysSince = lastBuy ? dayjs().diff(dayjs(lastBuy), 'day') : null
                    return (
                      <div key={c.id ?? i} className="flex items-center justify-between py-1.5 border-b border-border/40 last:border-0">
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-fg truncate">{c.name ?? c.phone ?? '—'}</p>
                          <p className="text-[10px] text-muted">{c.phone ?? ''}</p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <span className={clsx(
                            'text-[10px] font-semibold px-1.5 py-0.5 rounded-full',
                            (daysSince ?? 0) > 60 ? 'text-rose bg-rose/10' : 'text-yellow-400 bg-yellow-400/10'
                          )}>
                            {daysSince ? `${daysSince}d ago` : '—'}
                          </span>
                          <p className="text-[10px] text-muted mt-0.5">{fmt.compact(Number(c.totalSpent ?? 0))} spent</p>
                        </div>
                      </div>
                    )
                  })}
                  <div className="mt-2 text-[10px] text-muted flex items-center gap-1">
                    ...
                    Consider sending a re-engagement offer to these customers
                  </div>
                </div>
              )}
            </div>
