...
// ── Main Page ─────────────────────────────────────────────────────────────────
type Tab = 'stock' | 'movements' | 'valuation' | 'branches'
...
export default function InventoryPage() {
  ...
  const { data: allInvRaw } = useQuery({
    queryKey: ['inventory-all'],
    queryFn:  () => inventoryApi.list({ limit: 500 }),
    select:   (d: any) => d.data as any[],
    enabled:  tab === 'branches',
  })

  // Build matrix: productName → { branchId → { qty, threshold, variantId, branchId } }
  const matrix = useMemo(() => {
    if (!allInvRaw?.length) return { rows: [], branchIds: [] }
    const branchIds = [...new Set((allInvRaw).map((i: any) => i.branchId))] as string[]

    const productMap: Record<string, {
      name: string
      sku:  string
      rows: Record<string, { qty: number; threshold: number; variantId: string; branchId: string; branchName: string }>
    }> = {}

    for (const item of allInvRaw) {
      const key = item.variant?.product?.name ?? item.variantId
      if (!productMap[key]) {
        productMap[key] = {
      ...
      productMap[key].rows[item.branchId] = {
        qty:        item.quantity,
  ...
  const branchNameMap = useMemo(() => {
    ...
    for (const item of allInvRaw ?? []) {
      if (item.branchId && item.branch?.name) m[item.branchId] = item.branch.name
    }
    return m
  }, [allInvRaw])
  ...
  const TABS: { id: Tab; label: string; icon: any }[] = [
    ...
    { id: 'branches',  label: 'Branch Matrix',         icon: GitBranch  },
  ]
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 text-xs text-muted ml-auto">
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-jade inline-block" /> Yetarli
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-amber-400 inline-block" /> Kam
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-rose inline-block" /> Tugagan
              </span>
            </div>
          </div>
          ...
          <div className="card overflow-hidden p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-surface2">
                    <th className="text-left px-4 py-3 text-xs font-medium text-muted whitespace-nowrap sticky left-0 bg-surface2 z-10 min-w-[180px]">
                      Mahsulot
                    </th>
                    <th className="text-left px-3 py-3 text-xs font-medium text-muted whitespace-nowrap">SKU</th>
                    {matrix.branchIds.map(bid => (
                      <th key={bid} className="text-center px-4 py-3 text-xs font-medium text-muted whitespace-nowrap min-w-[100px]">
                        {branchNameMap[bid] ?? bid.slice(0, 8)}
                      </th>
                    ))}
                    <th className="text-center px-4 py-3 text-xs font-medium text-muted whitespace-nowrap">Jami</th>
                    <th className="px-3 py-3 text-xs font-medium text-muted whitespace-nowrap">Transfer</th>
                  </tr>
                </thead>
                <tbody>
                  {!allInvRaw && (
                    <tr>
                      <td colSpan={matrix.branchIds.length + 4} className="text-center py-10 text-muted">
                        Yuklanmoqda...
                      </td>
                    </tr>
                  )}
                  {filteredRows.length === 0 && allInvRaw && (
                    <tr>
                      <td colSpan={matrix.branchIds.length + 4} className="text-center py-10 text-muted">
                  ...
                  {filteredRows.map((row, ri) => {
                    const total = Object.values(row.rows).reduce((s, r) => s + r.qty, 0)
                    // Find best source branch (most stock) for quick transfer
                    const sourceBranch = Object.values(row.rows).sort((a, b) => b.qty - a.qty)[0]
                    return (
                      <tr key={ri} className="border-b border-border last:border-0 hover:bg-surface2/40 transition-colors">
                        <td className="px-4 py-2.5 font-medium text-fg sticky left-0 bg-surface z-10 hover:bg-surface2/60">
                          {row.name}
                        </td>
                        <td className="px-3 py-2.5 font-mono text-xs text-muted">{row.sku}</td>
                        {matrix.branchIds.map(bid => {
                          const cell = row.rows[bid]
                          if (!cell) {
                            return (
                              <td key={bid} className="px-4 py-2.5 text-center">
                                <span className="text-muted/40 text-xs">—</span>
                              </td>
                            )
                          }
                          const isLow  = cell.qty <= cell.threshold && cell.qty > 0
                          const isZero = cell.qty === 0
                          const isOk   = !isLow && !isZero
                          return (
                            <td key={bid} className="px-4 py-2.5 text-center">
                              <div className="inline-flex flex-col items-center gap-0.5">
                                <span className={clsx(
                                  'text-sm font-bold font-mono rounded-lg px-2.5 py-0.5',
                                  isZero ? 'text-rose   bg-rose/10' :
                                ...
                                {isLow && !isZero && (
                                  <span className="text-[9px] text-amber-400">min:{cell.threshold}</span>
                                )}
                              </div>
                            </td>
                          )
                        ...
                        <td className="px-3 py-2.5">
                          {sourceBranch && (
                            <button
                              onClick={() => setTransfer({
                                ...sourceBranch,
                                variantId: sourceBranch.variantId,
                                branchId:  sourceBranch.branchId,
                                quantity:  sourceBranch.qty,
                                branch:    { name: sourceBranch.branchName },
                                variant:   { product: { name: row.name }, sku: row.sku },
                              })}
                              className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1 px-2 py-1 rounded hover:bg-blue-400/10 transition-colors whitespace-nowrap"
                            >
                              <ArrowRightLeft size={11} /> Ko'chirish
                            </button>
                          )}
                        </td>
                      </tr>
          ...
          {filteredRows.length > 0 && (
            <div className="text-xs text-muted text-right">
              {filteredRows.length} mahsulot · {matrix.branchIds.length} filial
            </div>
          )}
        </div>
      )}
