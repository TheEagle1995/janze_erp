...
import {
  ...
  Printer, ArrowUpDown, ArrowUp, ArrowDown,
} from 'lucide-react'
...
function printReceipt(order: any) {
  const lines = (order.items ?? []).map((i: any) => {
    ...
    return `<tr>
      <td style="padding:3px 6px">${name}${variant ? ` <span style="opacity:.6">(${variant})</span>` : ''}</td>
      <td style="padding:3px 6px;text-align:center">${i.quantity}</td>
      <td style="padding:3px 6px;text-align:right">${Number(i.unitPrice).toLocaleString()}</td>
      <td style="padding:3px 6px;text-align:right">${Number(i.lineTotal ?? i.unitPrice * i.quantity).toLocaleString()}</td>
    </tr>`
  }).join('')

  const payments = (order.payments ?? []).map((p: any) =>
    `<tr><td style="padding:2px 6px;opacity:.7">${p.method}</td><td style="padding:2px 6px;text-align:right">${Number(p.amount).toLocaleString()}</td></tr>`
  ).join('')

  const html = `<!DOCTYPE html><html><head><meta charset="utf-8">
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:monospace;font-size:12px;width:280px;padding:12px;color:#000}
  h2{text-align:center;font-size:14px;margin-bottom:4px}
  .sub{text-align:center;font-size:10px;opacity:.6;margin-bottom:10px}
  hr{border:none;border-top:1px dashed #999;margin:8px 0}
  table{width:100%;border-collapse:collapse}
  th{font-size:10px;opacity:.6;padding:3px 6px;text-align:left;border-bottom:1px solid #ddd}
  .total{font-size:14px;font-weight:bold;text-align:right;padding:6px}
  .footer{text-align:center;font-size:10px;opacity:.5;margin-top:10px}
</style></head><body>
<h2>AVERO / JANZE</h2>
...
<hr>
<table><thead><tr>
  <th>Mahsulot</th><th style="text-align:center">Soni</th>
  <th style="text-align:right">Narx</th><th style="text-align:right">Jami</th>
</tr></thead><tbody>${lines}</tbody></table>
<hr>
${order.discountTotal > 0 ? `<div style="text-align:right;padding:2px 6px;opacity:.7">Chegirma: -${Number(order.discountTotal).toLocaleString()}</div>` : ''}
<div class="total">JAMI: ${Number(order.total).toLocaleString()} UZS</div>
<hr>
<table>${payments}</table>
<hr>
${order.cashier?.name ? `<div class="sub">Kassir: ${order.cashier.name}</div>` : ''}
<div class="footer">Xarid uchun rahmat!</div>
</body></html>`
...
// ── Sort helpers ───────────────────────────────────────────────────────────────
type OrderSortKey = 'createdAt' | 'total' | 'orderNumber' | 'status'
type SortDir      = 'asc' | 'desc'

function SortTh({ label, sortKey, active, dir, onSort }: {
  label: string; sortKey: OrderSortKey; active: boolean; dir: SortDir
  onSort: (k: OrderSortKey) => void
}) {
  return (
    <th onClick={() => onSort(sortKey)}
      className="text-left px-4 py-3 text-xs font-medium text-muted whitespace-nowrap cursor-pointer select-none hover:text-fg group">
      <span className="flex items-center gap-1">
        {label}
        {active
          ? dir === 'asc' ? <ArrowUp size={10} className="text-gold" /> : <ArrowDown size={10} className="text-gold" />
...
export default function OrdersPage() {
  ...
  const orders = [...rawOrders].sort((a: any, b: any) => {
    ...
    else if (sortKey === 'orderNumber') { av = a.orderNumber;   bv = b.orderNumber }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-surface2">
                <SortTh label={t.orders.orderNumber} sortKey="orderNumber" active={sortKey==='orderNumber'} dir={sortDir} onSort={toggleSort} />
                <SortTh label={t.common.date}        sortKey="createdAt"   active={sortKey==='createdAt'}   dir={sortDir} onSort={toggleSort} />
                <th className="text-left px-4 py-3 text-xs font-medium text-muted whitespace-nowrap">{t.common.cashier}</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-muted whitespace-nowrap">{t.orders.customer}</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-muted whitespace-nowrap">{t.orders.items}</th>
                <SortTh label={t.common.total}  sortKey="total"  active={sortKey==='total'}  dir={sortDir} onSort={toggleSort} />
                <th className="text-left px-4 py-3 text-xs font-medium text-muted whitespace-nowrap">{t.orders.payment}</th>
                <SortTh label={t.orders.status} sortKey="status" active={sortKey==='status'} dir={sortDir} onSort={toggleSort} />
                <th className="text-left px-4 py-3 text-xs font-medium text-muted whitespace-nowrap">{t.common.actions}</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            printReceipt(full)
                          } catch { printReceipt(o) }
                        }}
                        className="p-1.5 rounded hover:bg-surface2 text-muted hover:text-jade transition-colors" title="Chek chop etish">
                        <Printer size={13} />
                      </button>
