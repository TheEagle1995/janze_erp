...
import {
  ...
  Smartphone, TrendingDown, Zap, UserPlus, History, ShoppingBag, RotateCcw,
} from 'lucide-react'
...
export default function POSPage() {
  ...
  // Cache products to IndexedDB whenever online products load
  useEffect(() => {
    if (!onlineProducts?.length) return
    const toCache = onlineProducts.map((p: any) => ({
      id:       p.id,
      name:     p.name,
      barcode:  p.barcode ?? p.variants?.[0]?.barcode,
      price:    p.sellPrice ?? 0,
      stock:    p.variants?.reduce((s: number, v: any) =>
        s + (v.inventory?.find((i: any) => i.branchId === user?.branchId)?.quantity ?? 0), 0) ?? 0,
      ...
      variants: p.variants?.map((v: any) => ({
        id:    v.id,
        name:  v.name ?? p.name,
        price: v.priceOverride ?? p.sellPrice ?? 0,
        stock: v.inventory?.find((i: any) => i.branchId === user?.branchId)?.quantity ?? 0,
      })),
      updatedAt: Date.now(),
    }))
    offlineDB.saveProducts(toCache).catch(() => {})
  }, [onlineProducts, user?.branchId])

  // Load from IndexedDB when offline
  useEffect(() => {
    if (!isOffline) return
    offlineDB.getAllProducts().then(cached => {
      if (!cached.length) return
      // Convert cached format back to product format POSPage expects
      const mapped = cached.map(c => ({
        ...
        variants:  c.variants?.map(v => ({
          ...
          inventory:      [{ branchId: user?.branchId, quantity: v.stock }],
        })) ?? [],
      }))
      setOfflineProducts(mapped)
    })
  }, [isOffline, user?.branchId])

  // Active products: online data when connected, cached data when offline
  const products = isOffline ? offlineProducts : onlineProducts
    ...
    if (!navigator.onLine) {
      syncQueue.enqueue({
        items:         orderDto.items.map(i => ({
          productId:  i.variantId,   // best we have offline
          variantId:  i.variantId,
          qty:        i.quantity,
          price:      i.unitPrice,
          discount:   (i.discountFixed ?? 0) + (i.discountPct ?? 0),
        })),
        customerId:    cart.customerId ?? undefined,
        paymentMethod: (effectivePayments.find(p => Number(p.amount) > 0)?.method as any) ?? 'CASH',
        total,
        branchId:      user?.branchId,
      }).then(() => {
        const fakeOrder = {
        ...
        setSuccess({ ...fakeOrder, payments: effectivePayments })
        cart.clearCart()
        toast('🔴 Offline — sotuv navbatga qo\'shildi', { icon: '📋', duration: 4000 })
      })
      return
    }
  ...
  useEffect(() => {
    const PAYMENT_CYCLE: PayMethod[] = ['CASH', 'CARD', 'TRANSFER']

    const onKey = (e: KeyboardEvent) => {
      // Don't fire if typing in an input (except specific F-keys)
      const tag = (e.target as HTMLElement).tagName
      ...
      switch (e.key) {
        ...
        case 'F2':
          e.preventDefault()
          if (!inInput) setCameraOpen(true)
          break

        case 'F3':
          e.preventDefault()
          if (!inInput) { setReturnOrder(null); setReturnOpen(true) }
          break
        ...
        case 'F5':
          e.preventDefault()
          if (!inInput && cart.items.length > 0) {
            cart.clearCart()
            setPayments([{ method: 'CASH', amount: '' }])
            toast('🗑 Savatcha tozalandi', { icon: '✓', duration: 1500 })
          }
          break

        case 'F6':
          e.preventDefault()
          if (!inInput) setNewCustomerOpen(true)
          break

        case 'F8':
          // Cycle payment method: CASH → CARD → TRANSFER → CASH
          e.preventDefault()
          if (!inInput) {
            setPayments(prev => {
              if (prev.length !== 1) return prev
              const cur = prev[0].method as PayMethod
              const idx = PAYMENT_CYCLE.indexOf(cur)
              const next = PAYMENT_CYCLE[(idx + 1) % PAYMENT_CYCLE.length]
              toast(`💳 ${next}`, { duration: 1200 })
              return [{ method: next, amount: prev[0].amount }]
            })
          }
          break
        ...
        case 'F10':
          e.preventDefault()
          if (!inInput && !success) handleCheckout()
          break

        case 'F11':
          e.preventDefault()
          if (!inInput) setTab(t2 => t2 === 'history' ? 'products' : 'history')
          break
        ...
        case 'Escape':
          ...
          if (returnOpen)      { setReturnOpen(false);    break }
          if (cameraOpen)      { setCameraOpen(false);    break }
          if (newCustomerOpen) { setNewCustomerOpen(false); break }
          if (tab === 'variants') { setTab('products'); setSelectedProduct(null) }
          break

        default:
          // +/- keys: adjust last cart item quantity
          if (!inInput && cart.items.length > 0) {
            const last = cart.items[cart.items.length - 1]
            if (e.key === '+' || e.key === 'NumpadAdd') {
              e.preventDefault()
              cart.updateQty(last.variantId, last.quantity + 1)
            } else if ((e.key === '-' || e.key === 'NumpadSubtract') && last.quantity > 1) {
              e.preventDefault()
              cart.updateQty(last.variantId, last.quantity - 1)
            }
          }
    ...
    cart, payments, success, tab,
    cameraOpen, newCustomerOpen, showHotkeys, returnOpen,
    handleCheckout, setPayments,
  ])

  // ── Handle print ──────────────────────────────────────────
  const printerWidth = usePrinterStore(s => s.settings.width)

  const handlePrint = (order: any) => {
    const orderItems: any[] = order.items ?? cart.items
    const usedPayments = success?.payments ?? payments

    // Primary: XPrint (termal printer via WebSocket)
    xprint.printReceipt({
      brandName:   user?.branch?.name ?? 'JANZE ERP',
      ...
      payMethod:   usedPayments.find((p: any) => Number(p.amount) > 0)?.method ?? 'CASH',
      items:       orderItems.map((i: any) => ({
        name:    i.name      ?? i.productName ?? '—',
        variant: i.size      ?? i.variantName,
        qty:     i.quantity  ?? i.qty ?? 1,
        price:   i.unitPrice ?? i.price ?? 0,
        total:   i.lineTotal ?? (i.unitPrice ?? 0) * (i.quantity ?? 1),
      })),
      subtotal:    orderItems.reduce((s: number, i: any) => s + (i.lineTotal ?? (i.unitPrice ?? 0) * (i.quantity ?? 1)), 0),
      discountAmt: discount,
      total:       Number(order.total ?? total),
      cashGiven:   cashGiven > 0 ? cashGiven : undefined,
      change:      change    > 0 ? change    : undefined,
      width80mm:   printerWidth === '80mm',
    }).then(sent => {
      if (!sent) {
        // Fallback: old browser print (already called inside xprint.browserPrintReceipt)
        toast('📄 XPrint agenti yo\'q — brauzer chopi ishlatildi', { icon: 'ℹ️', duration: 3000 })
      }
    })
  }
  ...
  return (
    ...
    {returnOpen && (
      <ReturnModal
        initialOrder={returnOrder ?? undefined}
        onClose={() => { setReturnOpen(false); setReturnOrder(null) }}
        onSuccess={() => {
          setReturnOpen(false)
          setReturnOrder(null)
    ...
    {showHotkeys && (
      ...
      >
        <div
          className="bg-surface border border-border rounded-2xl shadow-2xl p-6 w-96"
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-sm">⌨️ Keyboard Shortcuts</h3>
            <button onClick={() => setShowHotkeys(false)} className="text-muted hover:text-fg">
              <X size={16} />
            </button>
          </div>
          <div className="space-y-1.5">
            {[
              ['F1',  'Ushbu yordamni ko\'rsatish / yashirish'],
              ['F2',  'Kamera skaneri'],
              ['F3',  'Qaytarish / Refund'],
              ['F4',  'Qidiruvga o\'tish'],
              ['F5',  'Savatchani tozalash'],
              ['F6',  'Yangi mijoz qo\'shish'],
              ['F8',  'To\'lov turini almashtirish'],
              ['F10', 'To\'lash (checkout)'],
              ['F11', 'Bugungi sotuvlar tarixi'],
              ['F12', 'Yangi sotuv'],
              ['Esc', 'Orqaga / yopish'],
              ['+',   'Oxirgi mahsulot miqdorini +1'],
              ['−',   'Oxirgi mahsulot miqdorini −1'],
            ].map(([key, label]) => (
              <div key={key} className="flex items-center gap-3 py-1 text-sm">
                <kbd className="px-2 py-0.5 rounded-md bg-surface2 border border-border font-mono text-xs font-semibold text-gold min-w-[40px] text-center">
                  {key}
                </kbd>
          ...
          <p className="text-xs text-muted mt-4 pt-3 border-t border-border">
            Klaviatura yorliqlari input maydoni tashqarisida ishlaydi
          </p>
        </div>
      </div>
    )}
        ...
        <div className="flex gap-2 mb-4">
          {tab !== 'history' && (
            <div className="relative flex-1">
              ...
              <input ref={searchInputRef} value={search} onChange={e => setSearch(e.target.value)} placeholder={t.pos.searchProducts} className="input pl-9" />
            </div>
          )}
          {tab === 'history' && (
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-border bg-surface2 text-muted hover:text-gold hover:border-gold/40 transition-all text-xs font-medium"
          >
            <kbd className="font-mono font-bold">F1</kbd>
          </button>
            ...
            {posHistory.map((o: any) => (
              <div key={o.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-surface2 border border-border/50 hover:border-border transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {o.status === 'COMPLETED' && (
                    <button
                      onClick={() => { setReturnOrder(o); setReturnOpen(true) }}
                      className="p-1 rounded-lg border border-rose/30 text-rose hover:bg-rose/10 transition-colors"
      ...
      <div className="w-80 flex flex-col bg-surface border border-border rounded-xl overflow-hidden flex-shrink-0">
        ...
        <div className="p-4 border-t border-border space-y-3">
          ...
          >
            ...
            {' '}
            {cart.items.length > 0 && isBalanced && overStockItems.length === 0 && (
              <kbd className="ml-1.5 text-[10px] font-mono font-semibold opacity-60 bg-white/10 px-1.5 py-0.5 rounded">
                F10
              </kbd>
            )}
          </button>
